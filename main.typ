...
Два выдающихся учёного заложили основы нейрофизиологии:

#image("images/img1.jpg", width: 80%)

*Герман фон Гельмгольц* (1821–1894) -- первым применил физику к изучению нервной системы. Его главное достижение: #highlight[измерение скорости нервного импульса] (~30-40 м/с). Это доказало, что нервные сигналы передаются не мгновенно, а с конечной скоростью -- нервная система работает как #highlight[информационная сеть]. Гельмгольц также разработал теорию трихроматии цветового зрения, показав, что восприятие #highlight[кодируется математически]. Его работы подтвердили: биологические процессы поддаются количественному описанию.

#image("images/img2.jpg", width: 80%)

*Чарльз Шеррингтон* (1857–1952) -- ввёл понятие #highlight[синапса] (место контакта между нейронами). Показал, что сигнал может быть возбуждающим или тормозящим. Описал принцип #highlight[постсинаптической интеграции]: нейрон суммирует входные сигналы и активируется только при достижении порога. Исследовал рефлексы, раскрыв механизм обратной связи. Его открытия стали прямой биологической основой для искусственных нейронов:
...
*Фрэнк Розенблатт* (1958) создал первый #highlight[перцептрон] -- искусственный нейрон, получающий входные сигналы, каждый умножается на вес, сумма подаётся на активационную функцию. Предложил простое правило обучения на основе ошибки. Однако перцептроны не решали нелинейно разделимые задачи (XOR).

*Многослойные перцептроны* (1980-е) с алгоритмом #highlight[обратного распространения ошибок] (backpropagation) решили эту проблему. Скрытые слои позволили моделировать сложные нелинейные зависимости. Ошибка на выходе распространяется назад, корректируя веса каждого нейрона. Это стало основой современного глубокого обучения.
...
*Принцип:* сеть скользит по изображению фильтром (маленький прямоугольник), выделяя признаки. На первых слоях -- простые детали (линии, углы), на следующих -- сложные формы (текстуры, объекты части), на последних -- целые объекты.
...
- Контроль посещаемости по лицам
...
*Революция:* вместо последовательной обработки (как RNN: слово → слово → слово), трансформер смотрит на #highlight[весь текст одновременно] через механизм #highlight[внимания]. Он определяет, какие слова связаны между собой, независимо от расстояния в тексте.
...
- Анализ сочинений на пониманию и оригинальность
- Автоматическое создание конспектов лекций
...
- Sigmoid: сигмоида формула $f(x) = 1/(1+exp(-x))$ -- часто на выходе для классификации
...
В ближайшие 5-10 лет каждый ученик получит #highlight[персонального цифрового помощника], который:
